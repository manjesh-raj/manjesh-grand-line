...
- [Working in this repository](#working-in-this-repository) - worktrees, the shared stash, the shared working tree, the suite's own preference domain
...
### The self-test suite writes its own preference domain, not the captain's

**This used to be the longest trap in this file, and P10 of the 2026-09-25
review closed it.** The account of what it cost - five separate incidents,
`FM_RUN_CONTRAST_TESTS` and `FM_RUN_DAYLIGHT_DRILL_SLICE2_TESTS` failing
intermittently on a clean tree, a sidecar file, a source guard, and a
`defaults read GrandLine` ritual before every full run - is in
[`48-test-hermeticity.md`](docs/history/48-test-hermeticity.md). What is left
is the rule.

`run-all-tests.sh` runs each suite as its own process, and the unbundled binary
has no bundle id, so its `UserDefaults` used to land in the **real** `GrandLine`
domain - the same one the captain's own preferences live in. About 50 suites
change the theme. Every mitigation built around that (save/restore in a
`defer`, a source guard for suites that forget, a sidecar for the interrupted
case) was a mitigation, because a `defer` does not run on a signal and `SIGKILL`
cannot be handled at all.

**Every preference this app reads or writes now goes through
`AppDefaults.store`**, which is `UserDefaults.standard` in the app the captain
runs and a **per-process** `UserDefaults(suiteName:)` in any `FM_RUN_*`
process. So a suite cannot leak into the real domain, an interrupted run leaves
nothing behind, and two concurrent passes are two domains rather than two
writers of one. `AppDefaultsSweep` removes the per-process domain at exit and
any a killed run left. Three things follow:

- **Reach for `AppDefaults.store`, never `UserDefaults.standard`**, for
  anything this app owns. The system's own keys are the exception and must stay
  on the real domain - `SystemAppearanceFollower` reads `AppleInterfaceStyle`,
  which belongs to macOS, and redirecting it would make a suite measure a
  preference nobody set.
- **`SelfTestDefaultsGuard` is now a check, not a repair.** It records the real
  domain's guarded keys at the start of a self-test process and says so loudly
  at exit if they moved. `Phase3PolishSelfTest.checkTheRealDomainIsNeverWritten`
  asserts the same thing by deliberately changing the theme and the font size
  and then reading the real domain.
- **`checkSuitesRestoreTheTheme` stays**, because a suite that changes the theme
  and does not put it back still leaves the *next case in its own process*
  measuring a theme nobody selected.

**Two shared things a full run still fights over, so do not run two
`./Scripts/run-all-tests.sh` passes at once.** Neither is `UserDefaults`:

- A **named `NSPasteboard` is machine-global**.
  ...
  pasteboard while the app is locked, and that name is one board per machine.
  A sibling pass writing the same board reads as a real app-lock leak - which
  is the last thing anyone wants to write off as flake. Measured in
  `fm/grandline-notification-ambient-expand-fix`: that check failed in a run
  overlapping a sibling pass and passed 4/4 standalone moments later. Check
  `pgrep -fl run-all-tests` before believing it.
- **The working tree itself**, which is the section above.

**And one direction every note here would lead you to rule out:
`FM_RUN_DAYLIGHT_DRILL_SLICE2_TESTS` passes inside `run-all-tests.sh` and fails
when invoked standalone**, with "the terminal never laid out ((0.0, 0.0, 0.0,
0.0)) - the frame check would be vacuous", i.e. its own vacuity guard rather
than a colour assertion. Confirmed pre-existing rather than caused by the branch
that found it. The practical rule: for a **window-backed** suite the runner is
the authority, a standalone red is not by itself evidence you broke something,
and the cheap disambiguation is to revert your own changed files to `HEAD~1`,
rebuild, and see whether it still fails.

Two domains still exist and are still worth not confusing, if you ever do read
one by hand:

    defaults read GrandLine                       # the unbundled binary's
    defaults read com.manjesh.grandline.native    # the packaged app's

(Both were renamed by `fm/grandline-rename-firstmate-cockpit-to-grand-line`; a
machine that has not run the renamed build still has its values under
...
| [`48-test-hermeticity.md`](docs/history/48-test-hermeticity.md) | The `UserDefaults` leak between suites: five incidents, the mitigations, and the per-process suite domain that replaced them |
