...
- **Verified** with `swift build` (clean debug and release, zero warnings in
  `Sources/GrandLine`), the release binary confirmed to carry no
...
- **Two pieces of collateral worth knowing about, both from the timeout
  itself.** A killed suite leaves the shared `GrandLine` domain dirty -
