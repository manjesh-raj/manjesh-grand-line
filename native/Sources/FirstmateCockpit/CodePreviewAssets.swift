...
//
// Where the Code Preview destination's web bundle lives, and how it is found.
//
// The bundle itself (`native/Vendor/Monaco/web/`) is a committed, vendored
// build of the real `monaco-editor` package - see that directory's README for
// provenance, licensing and how to regenerate it. Nothing here builds,
// downloads or updates it: this file only answers "where is it on this
// machine", and answers `nil` rather than crashing when it is missing.
//
// This is `WhiteboardAssets.swift` with one string changed, deliberately and
// not by accident: the two features have the identical problem (a real
// directory this app ships that has to be findable both from an assembled
// `.app` and from the `swift build`/`swift run` dev flow), so they get the
// identical three-step answer rather than a second invention.
//
//   1. `Bundle.main.resourceURL` - the assembled `.app`, where
//      `build_native_app.sh` copies the directory into `Contents/Resources`
//      exactly as it already does for `icon.icns`, `sre_kubectl_mcp.py` and
//      the Excalidraw bundle.
//   2. `FM_CODE_PREVIEW_WEB_DIR` - an explicit override, for a self-test or
//      for pointing a running app at a freshly rebuilt bundle.
//   3. A walk up from the current working directory looking for
//      `native/Vendor/Monaco/web` - the dev flow.
//
...
// lesson. A missing bundle has to degrade to a legible empty state, never to a
// crash on a destination the captain merely clicked.

import Foundation

enum CodePreviewAssets {

    /// The directory holding `index.html`, `code-preview.js` and
    /// `code-preview.css`. `nil` when no candidate has a readable
    /// `index.html`.
    static func webDirectory() -> URL? {
        ...
        if let override = ProcessInfo.processInfo.environment["FM_CODE_PREVIEW_WEB_DIR"] {
        ...
        for _ in 0..<6 {
            let candidate = URL(fileURLWithPath: dir, isDirectory: true)
                .appendingPathComponent("native/Vendor/Monaco/web", isDirectory: true)
            if isUsable(candidate) { return candidate }
    ...
    static let missingBundleMessage =
        "The Monaco bundle isn't on this machine. Run native/Scripts/build-monaco-web.sh " +
        "to regenerate native/Vendor/Monaco/web, or set FM_CODE_PREVIEW_WEB_DIR to point at it."
    ...
    static let bundleDirectoryName = "MonacoWeb"
