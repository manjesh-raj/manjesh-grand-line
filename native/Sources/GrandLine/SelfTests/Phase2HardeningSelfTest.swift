...
//
// `swift build && FM_RUN_PHASE2_HARDENING_TESTS=1 .build/debug/GrandLine`
//
...
enum Phase2HardeningSelfTest {
    ...
    private static func sourceDirectory() -> URL? {
        ...
        for _ in 0..<6 {
            let candidate = dir.appendingPathComponent("Sources/GrandLine")
            if FileManager.default.fileExists(atPath: candidate.appendingPathComponent("Subprocess.swift").path) {
                return candidate
            }
            let nested = dir.appendingPathComponent("native/Sources/GrandLine")
