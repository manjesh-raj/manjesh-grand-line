...
//
// `FM_RUN_WIDGET_SNAPSHOT_TESTS=1 .build/debug/GrandLine`.
...
enum WidgetSnapshotSelfTest {
    ...
    private static func checkTheAppGroupBranchIsGatedOnATeamID(_ check: (Bool, String) -> Void) {
        ...
        check(!GrandLineWidgetContainer.isTeamPrefixed("group.com.manjesh.grandline.native"),
              "an unprefixed App Group identifier is not Team-ID-prefixed")
        check(GrandLineWidgetContainer.isTeamPrefixed("A1B2C3D4E5.group.com.manjesh.grandline.native"),
        ...
        check(!GrandLineWidgetContainer.isTeamPrefixed("SHORT.group.com.manjesh.grandline.native"),
    ...
    private static func checkTheExtensionAndTheContractAgree(_ check: (Bool, String) -> Void) {
        ...
        if let text = try? String(contentsOf: script, encoding: .utf8) {
            check(text.contains("Sources/GrandLine/WidgetSharedContract.swift"),
